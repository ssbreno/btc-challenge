...
export class DepositAccountUseCase {
  ...
  async execute(dto: UpdateAccountDTO, req: any): Promise<Pick<Account, 'balance'>> {
    ...
    await this.sendDepositConfirmationEmail(req.name, dto.amount, updatedAccount.balance);
  ...
  private async createDepositTransaction(amount: number, account: Account) {
    ...
    await this.createTransactionsUseCase.createTransactions(transaction, account);
  }

  private async calculateTotalAmount(req: any): Promise<number> {
    const transactions = await this.findTransactionsUseCase.findTransactions(req);
  ...
  private async sendDepositConfirmationEmail(userName: string, amount: number, balance: number) {
    const formattedBalance = formatBalanceInBRL(balance);
    await sendEmail('ssobralbreno@gmail.com', 'Deposit Confirmation', 'deposit-template', { user: userName, amount, formattedBalance });
  }
}
